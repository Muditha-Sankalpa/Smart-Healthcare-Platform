...
export default function BookAppointment() {
  ...
  // Read specialty from query param (set by Symptom Checker)
  const recommendedSpecialty = searchParams.get("specialty") || "";

  const [step, setStep] = useState(0);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [specialtyFilter, setSpecialtyFilter] = useState(recommendedSpecialty);
  ...
  // All unique specialties for the filter dropdown
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return doctors.filter(d => d.specialty.toLowerCase() === specialtyFilter.toLowerCase());
  ...
  // When filter changes, reset doctor selection
  useEffect(() => {
    setForm(f => ({ ...f, doctorId: "", date: "" }));
  }, [specialtyFilter]);
              ...
              {step === 0 && (
                <div className="fade-up">
                  ...
                  {recommendedSpecialty && (
                    <div className="fade-up" style={{
                      background: "#EEEFFD", border: "1px solid #c7cae8",
                      ...
                      <div>
                        <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: "#122056", marginBottom: 2 }}>
                          AI Recommendation
                        </p>
                        <p style={{ margin: 0, fontSize: 13, color: "#5B65DC", lineHeight: 1.5 }}>
                          Based on your symptoms, we've pre-filtered doctors to{" "}
                          <strong>{recommendedSpecialty}</strong>. You can change this below.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {/* Specialty Filter */}
                  <Field label="Filter by Specialty">
                    <div style={{ position: "relative" }}>
                      <select
                        ...
                        style={{
                          ...
                          background: specialtyFilter ? "#EEEFFD" : "#fff",
                          cursor: "pointer",
                          fontWeight: specialtyFilter ? 600 : 400,
                        }}
                        onFocus={e => (e.target.style.borderColor = "#5B65DC")}
                        onBlur={e => (e.target.style.borderColor = specialtyFilter ? "#5B65DC" : "#dde0f0")}
                      >
                    ...
                    {specialtyFilter && (
                      <p style={{ margin: "5px 0 0", fontSize: 11, color: "#5B65DC" }}>
                        Showing {filteredDoctors.length} doctor{filteredDoctors.length !== 1 ? "s" : ""} for <strong>{specialtyFilter}</strong>
                        {" · "}
                        <button
                          onClick={() => setSpecialtyFilter("")}
                          style={{ background: "none", border: "none", color: "#5B65DC", cursor: "pointer", fontSize: 11, padding: 0, textDecoration: "underline" }}
                        >
                          Clear filter
                        </button>
                      </p>
                  ...
                  <Field label="Select Doctor" required>
                    <SelectInput value={form.doctorId} onChange={set("doctorId")}>
                      ...
                      {filteredDoctors.length === 0 && (
                        <option value="" disabled>No doctors found for this specialty</option>
                      )}
                    </SelectInput>
                  </Field>
                  ...
                  {selectedDoctor && (
                    <div style={{
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ margin: 0, fontSize: 12, color: "#5B65DC" }}>
                          {selectedDoctor.specialty}
                          {selectedDoctor.experience && ` · ${selectedDoctor.experience} yrs exp`}
                        </p>
                      </div>
                      {selectedDoctor.consultationFee && (
                        <div style={{ textAlign: "right" }}>
                          <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "#122056" }}>
                            LKR {selectedDoctor.consultationFee.toLocaleString()}
                          </p>
                          <p style={{ margin: 0, fontSize: 11, color: "#8a90b8" }}>consult fee</p>
                        </div>
                      )}
                    </div>
                  )}
                ...
                <div className="fade-up">
                  ...
                  <div style={{
                    ...
                    <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 36, marginBottom: 4 }}>
                      LKR {selectedDoctor?.consultationFee?.toLocaleString() || "2,500"}.00
                    </div>
