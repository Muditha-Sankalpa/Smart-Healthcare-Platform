...
import { Search, Star, Calendar, MapPin, Check, X, Loader2, AlertCircle } from 'lucide-react';
...
const AdminDoctors = () => {
  ...
  const fetchDoctors = async (q = '') => {
    try {
      setLoading(true);
      // Call admin endpoint to get all doctors
                    ...
                    <tr key={doctor._id} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          {/* Toggle Verification */}
                          <button
                            onClick={() => handleToggleVerify(doctor._id, doctor.verified)}
                            className={`p-2 rounded-lg border transition ${
                              doctor.verified
                                ? 'bg-green-100 border-green-200 text-green-700 hover:bg-green-200'
                                : 'bg-yellow-100 border-yellow-200 text-yellow-700 hover:bg-yellow-200'
                            }`}
                            title={doctor.verified ? 'Unverify doctor' : 'Verify doctor'}
                          >
                            {doctor.verified ? <Check size={14} /> : <Check size={14} />}
                          </button>
                        </div>
                      </td>
                    </tr>
