...
import { getProfile, updateProfile, uploadReport, getHistory } from '../../api/patientApi';
import { Button, Card, PatientNavBar } from '../../components/shared';
  ...
  const UploadWidget = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 38, height: 38, borderRadius: '50%', border: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
            <path d="M12 15V8M9 11l3-3 3 3" stroke="#4b5563" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
            <path d="M20 16.5A3.5 3.5 0 0 0 17 10h-1.2A5 5 0 1 0 6 16.5" stroke="#4b5563" strokeWidth="1.7" strokeLinecap="round" />
          </svg>
        </div>
        <div>
          <div style={{ fontSize: 16, fontWeight: 600, color: '#111827' }}>Upload files</div>
      ...
      <div style={{ border: '1.5px dashed #d1d5db', borderRadius: 12, padding: '24px 14px', textAlign: 'center', background: '#fafafa' }}>
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" style={{ display: 'block', margin: '0 auto 8px' }}>
          <path d="M12 15V8M9 11l3-3 3 3" stroke="#4b5563" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M20 16.5A3.5 3.5 0 0 0 17 10h-1.2A5 5 0 1 0 6 16.5" stroke="#4b5563" strokeWidth="1.7" strokeLinecap="round" />
        </svg>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f4f6fb' }}>
      ...
      <div style={{ flex: 1, padding: 28, overflowY: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20 }}>
            ...
            <Card className="flex flex-col gap-3 rounded-2xl">
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, paddingBottom: 16, borderBottom: '0.5px solid #e5e7eb' }}>
                <div style={{ width: 60, height: 60, borderRadius: '50%', background: '#122056', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 22, fontWeight: 500 }}>
                  {initials}
                </div>
                ...
                <div style={{ display: 'flex', gap: 14 }}>
                  {['email', 'sms'].map((pref) => (
                    <label key={pref} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: '#374151', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form.notificationPreference?.includes(pref) || false}
                        onChange={() => editMode && toggleNotifPref(pref)}
                        disabled={!editMode}
                      />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                ...
                <Card className="rounded-2xl">
                  <div style={{ fontSize: 13, fontWeight: 500, color: '#122056', marginBottom: 12 }}>Upload medical reports</div>
                  <UploadWidget />
                </Card>

                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ background: '#e8f0fe', border: '0.5px solid #b5d4f4', borderRadius: 16, padding: 16, flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: '#0c447c', marginBottom: 4 }}>Make appointment</div>
                    <div style={{ fontSize: 11, color: '#185fa5', marginBottom: 12 }}>Book a session with a doctor</div>
                    <Button variant="primary">Book now</Button>
                  </div>
                  <div style={{ background: '#e1f5ee', border: '0.5px solid #9fe1cb', borderRadius: 16, padding: 16, flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: '#085041', marginBottom: 4 }}>Notifications</div>
                    <div style={{ fontSize: 11, color: '#0f6e56', marginBottom: 12 }}>View your recent alerts</div>
                    <Button onClick={() => setActiveNav('notifications')} variant="outline">View</Button>
                  </div>
                </div>
              </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ marginLeft: 'auto' }}>
                    <select
                      ...
                      style={{ border: '1px solid #d1d5db', borderRadius: 8, padding: '5px 10px', fontSize: 12 }}
                    >
                      ...
                      <div key={i} style={{ border: '0.5px solid #e5e7eb', borderRadius: 10, padding: '12px 14px' }}>
                                ...
                                >
                                  <div style={{ minWidth: 0 }}>
                                    ...
                                    <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>
                                      Status: {item.status} | {item.scheduledTime ? new Date(item.scheduledTime).toLocaleDateString() : ''}
                                    </div>
                                  </div>
                                  <span aria-hidden="true" style={{ display: 'flex', alignItems: 'center', color: '#9ca3af' }}>
                                    {isExpanded ? (
                                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                                ...
                                {isExpanded && (
                                  <div style={{ marginTop: 10, paddingTop: 10, borderTop: '0.5px solid #e5e7eb', display: 'grid', gap: 6 }}>
                                    <div style={{ fontSize: 12, color: '#374151' }}>
                                      <span style={{ color: '#6b7280' }}>Session ID:</span> {item.sessionId || '—'}
                                    </div>
                                    <div style={{ fontSize: 12, color: '#374151' }}>
                                      <span style={{ color: '#6b7280' }}>Appointment ID:</span> {item.appointmentId || '—'}
                                    </div>
                                    <div style={{ fontSize: 12, color: '#374151' }}>
                              ...
                              <div>
                                ...
                                >
                                  <div style={{ minWidth: 0 }}>
                                    ...
                                    <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>
                                      {medsSummary} | {item.createdAt ? new Date(item.createdAt).toLocaleDateString() : ''}
                                    </div>
                                  </div>
                                  <span aria-hidden="true" style={{ display: 'flex', alignItems: 'center', color: '#9ca3af' }}>
                                    {isExpanded ? (
                                ...
                                {isExpanded && (
                                  <div style={{ marginTop: 10, paddingTop: 10, borderTop: '0.5px solid #e5e7eb', display: 'grid', gap: 8 }}>
                                    ...
                                    <div style={{ fontSize: 12, color: '#374151' }}>
                                      <span style={{ color: '#6b7280' }}>Notes:</span> {item.notes || '—'}
                                    </div>
                                    <div style={{ fontSize: 12, color: '#374151' }}>
                                      <span style={{ color: '#6b7280' }}>Follow-up Date:</span>{' '}
                                        ...
                                        <div style={{ display: 'grid', gap: 6 }}>
                                          {item.medications.map((m, idx) => (
                                            <div key={idx} style={{ padding: '8px 10px', borderRadius: 10, background: '#fafafa', border: '0.5px solid #e5e7eb' }}>
                                              ...
                                              <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>
                                                {[m?.dosage && `Dosage: ${m.dosage}`, m?.frequency && `Frequency: ${m.frequency}`, m?.duration && `Duration: ${m.duration}`]
                                                  .filter(Boolean)
                                                  .join(' • ') || '—'}
                                              </div>
                                              {m?.instructions && (
                        ...
                        {activeTab === 'reports' && (
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            {item.fileUrl && (
                              <a href={item.fileUrl} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: '#122056', textDecoration: 'underline' }}>View</a>
                            )}
                          </div>
                        )}
                      </div>
        ...
        {activeNav === 'history' && (
          <Card className="rounded-2xl">
            <div style={{ fontSize: 16, fontWeight: 500, color: '#122056', marginBottom: 16 }}>Medical history</div>
            ...
            <input
              placeholder="Search..."
              onChange={(e) => setHistoryFilter(e.target.value)}
              style={{ width: '100%', border: '1px solid #d1d5db', borderRadius: 10, padding: '8px 12px', fontSize: 13, marginBottom: 14, boxSizing: 'border-box' }}
            />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {filterItems(history[activeTab] || []).length === 0 ? (
                          ...
                          <>
                            ...
                            >
                              <div style={{ minWidth: 0 }}>
                                ...
                                <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>
                                  {item.scheduledTime ? new Date(item.scheduledTime).toLocaleDateString() : ''}
                                </div>
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 10, background: item.status === 'COMPLETED' ? '#d1fae5' : '#f3f4f6', color: item.status === 'COMPLETED' ? '#065f46' : '#6b7280' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          {activeTab === 'prescriptions' && (
                            <>
                              <div style={{ fontSize: 13, color: '#111827', fontWeight: 500 }}>{item.diagnosis}</div>
                              <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>{item.medications?.map((m) => m.name).join(', ')}</div>
                            </>
                          )}
                          {activeTab === 'reports' && (
                            <>
        ...
        {activeNav === 'upload' && (
          <div style={{ maxWidth: 500 }}>
            <Card className="rounded-2xl">
              <div style={{ fontSize: 16, fontWeight: 500, color: '#122056', marginBottom: 16 }}>Upload medical reports</div>
              <UploadWidget />
            </Card>
          </div>
        )}
        ...
        {activeNav === 'notifications' && (
          <div style={{ maxWidth: 560 }}>
            <Card className="rounded-2xl">
              <div style={{ fontSize: 16, fontWeight: 500, color: '#122056', marginBottom: 16 }}>My notifications</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ border: '0.5px solid #b5d4f4', borderRadius: 10, padding: '14px 16px', background: '#e8f0fe' }}>
                  ...
                  <div style={{ fontSize: 12, color: '#185fa5', marginTop: 3 }}>You have an upcoming appointment. Check your schedule.</div>
                </div>
                <div style={{ border: '0.5px solid #9fe1cb', borderRadius: 10, padding: '14px 16px', background: '#e1f5ee' }}>
                  ...
                  <div style={{ fontSize: 12, color: '#0f6e56', marginTop: 3 }}>Your virtual session is scheduled. Check your consultations.</div>
                </div>
                {history.prescriptions?.length > 0 && (
                  <div style={{ border: '0.5px solid #e5e7eb', borderRadius: 10, padding: '14px 16px' }}>
                    ...
                    <div style={{ fontSize: 12, color: '#6b7280', marginTop: 3 }}>
                      You have {history.prescriptions.length} prescription(s). View in Medical History.
                    </div>
                  </div>
                )}
              </div>
