...
const SessionsPageAdmin = () => {
  ...
  const fetchSessions = async () => {
    try {
      ...
      if (err.response?.status === 401) {
        ...
        setError(
          "Route not found. Ensure the Gateway and Telemedicine service are both running.",
        );
      } else {
  ...
  const handleCancel = async (sessionId) => {
    try {
      await API.put(`/telemedicine/${sessionId}/cancel`);
      fetchSessions();
    } catch (err) {
      console.error(err);
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary">
            Manage and monitor all virtual consultations
          </p>
        </div>
        <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-secondary/50 border-b border-secondary">
                  <th className="px-6 py-4 text-sm font-semibold text-primary uppercase tracking-wider">
                    Session Info
                  </th>
                  <th className="px-6 py-4 text-sm font-semibold text-primary uppercase tracking-wider">
                    Participants
                  ...
                  <th className="px-6 py-4 text-sm font-semibold text-primary text-right uppercase tracking-wider">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-secondary">
                {filteredSessions.length === 0 ? (
                  <tr>
                    <td
                      colSpan="5"
                      className="px-6 py-10 text-center text-text-secondary italic"
                    >
                        ...
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <div className="relative">
                            <button
                              ...
                              className="p-2 text-gray-400 hover:text-primary rounded-lg border border-transparent hover:border-secondary transition-all"
                            >
                              <MoreVertical size={18} />
                            </button>

                            {openMenuId === session.sessionId && (
                              <div className="absolute right-0 mt-2 w-44 bg-white border border-secondary rounded-lg shadow-lg z-10">
                                <button
                                  onClick={() => {
                                ...
                                <button
                                  onClick={() => {
                                    handleComplete(session.sessionId);
                                    setOpenMenuId(null);
                                  }}
                                  className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100"
                                >
                                ...
                                <button
                                  onClick={() => {
                                    handleDelete(session.sessionId); // <-- this is your cancel (delete)
                                    setOpenMenuId(null);
                                  }}
